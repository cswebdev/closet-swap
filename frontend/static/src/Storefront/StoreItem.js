...
function StoreItem(setClothingItem) {
   ...
   //       const response = await fetch(`/api_v1/closet/images/`);
   //       if (!response.ok) {
   //          throw new Error("Network response not ok");
   //       }
   //       const data = await response.json();
   ...
   // const itemListingsHTML = itemListings.map((item) => {
   //    return (
   //       <Row className="row " key={nanoid()}>
   //          <Col className="col p-1 m-0">
   //             <Card style={{ width: "18rem" }}>
   //                <Card.Img variant="top" src={item.image_url.url} />
      ...
      itemListingsHTML.push(
         <Row className="row" key={nanoid()}>
            {itemListings.slice(i, i + 3).map((item) => (
               <Col className="col p-1 m-0" key={item.id}>
                  <Card style={{ width: "18rem" }}>
                     <Card.Img
                        variant="top"
